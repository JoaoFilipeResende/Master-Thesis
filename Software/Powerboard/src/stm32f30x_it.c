...
void SPI1_IRQHandler()
{
	...
	if (SPI_I2S_GetITStatus(SPI1, SPI_I2S_IT_TXE) == SET)
	{
		...
		if (xQueueReceiveFromISR( xQueueSPI1_TX, &TxData, &pxHigherPriorityTaskWoken ) == pdTRUE)
		{
	...
	if(SPI_I2S_GetITStatus(SPI1, SPI_I2S_IT_RXNE) == SET)
	{
		...
		RxData = SPI_ReceiveData8(SPI1);

		if( xQueueSPI1_RX != 0)
		{
			xQueueSendToBackFromISR( xQueueSPI1_RX, &RxData, &pxHigherPriorityTaskWoken );
			if( pxHigherPriorityTaskWoken == pdTRUE )
				taskYIELD(); //Forces a context switch before exit the ISR.
		}
